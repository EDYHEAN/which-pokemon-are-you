...
import s from './PokemonSelector.module.css'
...
// ── Main component ────────────────────────────────────────────
export default function PokemonSelector({ isNight, setIsNight, onBack }) {
  ...
  const stars = useRef(
    Array.from({ length: 40 }, (_, i) => ({
      id: i, top: `${Math.random() * 92}%`, left: `${Math.random() * 100}%`,
      size: 1 + Math.random() * 1.5, duration: `${2 + Math.random() * 3}s`, delay: `${Math.random() * 4}s`,
    }))
  ).current
  ...
  return (
    <main className={`${s.page} ${isNight ? s.night : s.day}`}>
      ...
      <button className={s.toggle} onClick={() => setIsNight(n => !n)}
